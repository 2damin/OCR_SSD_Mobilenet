...
__global__ void paddingset(float *image, float *tmp, const int depthwiseOutputW, const int col, const int depth, const int padding)
{
	int idxRow = threadIdx.x + blockDim.x * blockIdx.x;
	int idxCol = threadIdx.y + blockDim.y * blockIdx.y;
	int depthBoundary = idxCol / col;
	int tmpSize = col + 2 * padding;
	//printf("(%d, %d) \n", threadIdx.y, blockIdx.y);
	if (idxRow < col && idxCol < col * depth)
	{
		tmp[depthBoundary*tmpSize*tmpSize + (idxCol - depthBoundary*col + padding)*(tmpSize)+(idxRow + padding)] = image[depthBoundary*col*col + (idxCol-depthBoundary*col)* col + idxRow];
...
void padding_f(float *image, float *tmp, const int depthwiseOutputW, const int col, const int depth, const int padding, dim3 threads_per_block, dim3 num_of_blocks) {
	paddingset << <  num_of_blocks, threads_per_block >> > (image, tmp, depthwiseOutputW, col, depth, padding);
}
	...
	const int depthwiseOutputW, const int padding, const int stride, const int blocksPerChannel)
{
	...
	//int idxXSrcPerChannel = (int)fminf((float)(idxXperChannel), (float)depthwiseOutputW);
	//int idxYSrcPerChannel = (int)fminf((float)(idxYperChannel), (float)depthwiseOutputW);
	int idxChannel = ((int)(blockIdx.x) / blocksPerChannel) % depth;
	//int idxN = ((int)(blockIdx.x) / blocksPerChannel) / depth;
